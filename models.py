...
class MaterialType(Enum):
    SOLVENT = "Solvent"
    LITHIUM_SALT = "LithiumSalt"
    ADDITIVE = "Additive"


# 氢键能力枚举
class HydrogenBonding(str, Enum):
    STRONG = "strong"
    WEAK = "weak"
    NONE = "none"
...
class MaterialModel(BaseModel):
    ...
    # fraction: float = Field(default=0, ge=0, le=100, description="组成百分比 (%)")
...
# 锂盐数据模型
class LithiumSaltModel(MaterialModel):
    ...
    material_type: MaterialType = MaterialType.LITHIUM_SALT
...
class AdditiveModel(MaterialModel):
    ...
    concentration: float = Field(..., ge=0, le=100, description="浓度 (wt%)")
    ...
    def validate_concentration(cls, value):
        if not 0 <= value <= 100:  # wt% 范围为 0-100
            raise ValueError(
                f"Concentration must be between 0 and 100 wt%, got {value}"
            )
        return value
...
# 类型变量，用于泛型设计
T = TypeVar("T", bound=MaterialModel)
...
class Component(BaseModel, Generic[T]):
    ...
    @field_validator("overall_fraction", mode="after")
    @classmethod
    def ensure_overall_fraction(cls, v, info):
        """确保锂盐、添加剂整体占比已提供"""
        values = info.data
        # 判断当前材料类型是否为添加剂或锂盐
        if (
            ...
            in {
                ...
                MaterialType.LITHIUM_SALT,
            }
            and v is None
        ):
            raise ValueError("锂盐和添加剂必须提供整体占比")

        return v

    @field_validator("relative_fraction", mode="after")
    @classmethod
    def ensure_relative_fraction(cls, v, info):
        """确保溶剂相对占比已提供"""
        values = info.data
        # 判断当前材料类型是否为溶剂
        if values["material"].material_type == MaterialType.SOLVENT and v is None:
            raise ValueError("溶剂必须提供相对占比")
...
class ElectrolyteModel(BaseModel):
    ...
    lithium_salts: list[Component[LithiumSaltModel]] = Field(
        ..., description="锂盐列表"
    )
    solvents: list[Component[SolventModel]] = Field(..., description="溶剂列表")
    additives: list[Component[AdditiveModel]] = Field(
        default_factory=list, description="添加剂列表"
    )
    ...
    def compute_fraction(self):
        ...
        # 计算锂盐总占比
        lithium_salt_fraction = sum(
            component.overall_fraction
            for component in self.lithium_salts
        ...
        # 计算溶剂总占比
        solvent_fraction = 100 - lithium_salt_fraction - additive_fraction

        # 计算锂盐相对占比
        for component in self.lithium_salts:
            if component.overall_fraction is not None:
                component.relative_fraction = round(
                    component.overall_fraction / lithium_salt_fraction * 100, 2
                )
        # 计算添加剂相对占比
        for component in self.additives:
            if component.overall_fraction is not None:
        ...
        if solvent_relative_fraction != 100:
